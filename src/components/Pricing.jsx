    ...
    <section id="pricing" className="relative w-full bg-slate-950 py-20 text-white">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-slate-300">Pick the plan that matches your goals. No hidden fees, ever.</p>
        </div>
          ...
          {tiers.map((t, i) => (
            <motion.div
              ...
              className={`relative rounded-2xl border p-6 backdrop-blur ${
                t.highlight
                  ? 'border-cyan-400/30 bg-gradient-to-b from-cyan-500/10 to-cyan-400/0'
                  : 'border-white/10 bg-gradient-to-b from-white/5 to-white/0'
              }`}
            >
              {t.highlight && (
                <span className="absolute right-4 top-4 rounded-full bg-cyan-400/20 px-3 py-1 text-xs font-semibold text-cyan-200 ring-1 ring-cyan-400/30">
                  Most popular
                </span>
              )}
              ...
              <div className="mt-2 flex items-end gap-1">
                ...
                <span className="mb-1 text-slate-300">{t.cadence}</span>
              </div>
